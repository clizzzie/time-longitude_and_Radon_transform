...
def main():
    ...
    # year_start, year_end =  -88000, -78000            # ggf100k
    ...
    # year_start, year_end =  -898000, -698000        # -810000, -760000 GGFMB
    ...
    # # year_increments, year_window = 0.005, 0.05      # LEDT002
    # year_increments, year_window = 200, 4000
...
def reverse_flux_from_TL_for_GGFMB(TL: np.ndarray, kas):
    """  Sums the reverse flux patches across the longitudinal axis on the time-longitude plot
    """

    kas_reversal_index = np.abs(kas-780).argmin()
    print('this is where kas reversal index is located:', kas[kas_reversal_index], kas_reversal_index)

    # normal state of polarity
    TL_max_5percent_Normal = np.mean(TL[ 0:kas_reversal_index, :])*0.1
    print('TL 10% mean: ', TL_max_5percent_Normal)
    reverse_flux_N = np.where(TL[0:kas_reversal_index,:] >= TL_max_5percent_Normal)
    reverse_TL_N = np.zeros(TL.shape)
    reverse_TL_N[reverse_flux_N[0],reverse_flux_N[1]] = TL[reverse_flux_N[0], reverse_flux_N[1]]
    reverse_flux_time_Normal = np.abs(reverse_TL_N).sum(axis=1)

    # reverse state of polarity
    TL_max_5percent_Reverse = np.mean(TL[kas_reversal_index:-1, :])*0.1
    print('TL 10% mean: ', TL_max_5percent_Reverse)
    reverse_flux_R = np.where(TL[kas_reversal_index:-1, :] <= TL_max_5percent_Reverse)
    reverse_TL_R = np.zeros(TL.shape)
    reverse_TL_R[reverse_flux_R[0],reverse_flux_R[1]] = TL[reverse_flux_R[0], reverse_flux_R[1]]
    reverse_flux_time_Reversal = np.abs(reverse_TL_R).sum(axis=1)

    return reverse_flux_time_Normal, reverse_flux_time_Reversal
...
def plots_reverse_flux_and_TL(TL: np.ndarray, years: np.ndarray):
    ...
    if model == "GGFMB":
        reverse_flux_time_Normal, reverse_flux_time_Reversal = reverse_flux_from_TL_for_GGFMB(TL, kas)
        kas_reversal_index = np.abs(kas-780).argmin()
        ax00.plot(reverse_flux_time_Normal, kas, 'darkred')
        # ax00.plot(np.flip(reverse_flux_time_Reversal), kas, 'darkblue')
        # ax00.plot(reverse_flux_time_Normal[0:kas_reversal_index], kas[0:kas_reversal_index], 'darkred')
        # ax00.plot(reverse_flux_time_Reversal[kas_reversal_index:], kas[kas_reversal_index:], 'darkblue')
    else:
        if latitude > 0:
            ax00.plot(reverse_flux_from_TL(TL), kas, 'darkred')
        else:
            ax00.plot(reverse_flux_from_TL(TL), kas, 'steelblue')
    ...
    if model == "GGFMB":
        ax00.set_yticks([700, 725, 750, 775, 800, 825, 850, 875, 900])
        ax00.axhspan(887.5, 888.5 , facecolor= 'gray', alpha = 0.4)
        ax00.axhspan(799, 770 , facecolor= 'gray', alpha = 0.4)
        fontsize_word = 20
        fontsize_tick = 15
        fontsize_small_word = 15
        ax00.set_ylabel('Age (ka)', fontsize = fontsize_word)
    ...
    ax01a = ax01.twiny()          # for the tickmarks above the TL plot
    ...
    for axis in ['top','bottom','left','right']:
        ...
        ax01a.spines[axis].set_linewidth(0.25)            # for the tickmarks above the TL plot

    if model == 'GGF100k' and latitude == 55:
        ax01a.set_xlim(longitude_start, longitude_end)
        ...
        ax01a.set_xticks(GGF100k_long[lat_index])
        ax01a.tick_params(labeltop = False)
        ax01a.tick_params(width = 2, color = 'dodgerblue')   # 'dodgerblue' 'darkgreen' 'indianred' 'firebrick'
    ...
    if model == 'GGFSS70' and latitude == 55:
        ...
        ax01a.set_xticks(GGFSS70_long[lat_index])
        ax01a.tick_params(labeltop = False)
        ax01a.tick_params(width = 2, color = 'indianred')   # 'dodgerblue' 'darkgreen' 'indianred' 'firebrick'
    ...
    if model == 'LSMOD.2' and latitude == 55:
        ax01a.set_xlim(longitude_start, longitude_end)
        LSMOD2_mean_lat_long = np.loadtxt('/Users/nclizzie/Documents/Paleomagnetic_data/LSMOD.2_mean_lat_long.txt')
        LSMOD2_mean_lat      = LSMOD2_mean_lat_long[:,0]
        LSMOD2_mean_long     = LSMOD2_mean_lat_long[:,1]
        lat_index = [index for index,value in enumerate(LSMOD2_mean_lat) if value < 75 and value > 35 ]
        print(LSMOD2_mean_long[lat_index])

        ax01a.set_xticks(LSMOD2_mean_long[lat_index])
        ax01a.tick_params(labeltop = False)
        ax01a.tick_params(width = 2, color = 'darkgreen')   # 'dodgerblue' 'darkgreen' 'indianred' 'firebrick'
    ...
    # elif    model == 'GGFMB':
    #     cbar = plt.colorbar(TL_show, aspect = 30)
...
def plot_moving_window_and_max_signal_and_tilt_and_moment(GC_Dict: dict, moving_window, max_signal, years, drift, mov_wind_years_cent, year_increments, year_window, dt):
    ...
    if model == 'ledt002':
        ...
        if latitude == 55:
            # Westward
            ax011.plot([-0.082, -0.082], [33, 31],  ls="--", c=color_3, linewidth = 1.5)
            ax011.plot([-0.067, -0.067], [37, 33],  ls="--", c=color_3, linewidth = 1.5)
            ax011.plot([-0.082, -0.082], [38, 36],  ls="--", c=color_3, linewidth = 1.5)
            ax011.plot([-0.04, -0.04], [43, 40],  ls="--", c=color_3, linewidth = 1.5)
            # ax011.plot([-(360/7000), -(360/10000)], [40, 48],  ls="--", c=color_3, linewidth = 1.5)

            # Eastward
            ax011.plot([0.1, 0.1], [39, 44],  ls="--", c=color_1, linewidth = 1.5)
            ax011.plot([0.12, 0.12], [46, 50],  ls="--", c=color_1, linewidth = 1.5)

            # # Filter
            # ax011.axvspan(-180/4000, 180/4000 , facecolor= 'gray', alpha = 0.1)

        if latitude == -55:
            # Eastward
            ax011.plot([0.05, 0.05], [37, 43],  ls="--", c=color_1, linewidth = 1.5)
        ...
        if latitude == 55:
            # Westward
            ax011.plot([-0.1838, -0.1838], [41, 39],  ls="--", c=color_3, linewidth = 1.5)
        ...
        if latitude == -55:
            # Eastward
            ax011.plot([0.09, 0.09], [32, 39],  ls="--", c=color_1, linewidth = 1.5)

            # Westward
            ax011.plot([-0.08, -0.08], [38, 43.5],  ls="--", c=color_3, linewidth = 1.5)
        ...
        if latitude == 55:
            # Westward
            ax011.plot([-0.08, -0.08], [38, 43],  ls="--", c=color_3, linewidth = 1.5)
            # Eastward
            ax011.plot([0.082, 0.082], [40, 44],  ls="--", c=color_1, linewidth = 1.5)


        if latitude == -55:
            # Westward
            ax011.plot([-0.081, -0.081], [39, 43],  ls="--", c=color_3, linewidth = 1.5)
